...
def DownloadMode(urls_file, missing_urls_file, downloads_dir, request_parallelism, timestamp_exactness):
    ...
    while (len(urls_left_todownload) != 0):
        ...
        results = p.imap_unordered(DownloadMapper,
                                   zip(urls_left_todownload, repeat(downloads_dir), repeat(timestamp_exactness)))
...
def main():
    ...
    urls_file_to_download = "data/xsum/XSum-WebArxiveUrls.txt"
    missing_urls_file = "data/xsum/XSum-WebArxiveUrls.missing.txt"
    downloads_dir = "./xsum-raw-downloads"
    print('Creating the download directory.')
    os.system("mkdir -p " + downloads_dir)

    DownloadMode(urls_file_to_download, missing_urls_file, downloads_dir, args.request_parallelism,
                 args.timestamp_exactness)
